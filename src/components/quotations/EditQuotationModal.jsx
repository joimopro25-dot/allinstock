...
export function EditQuotationModal({ isOpen, onClose, onSuccess, quotation }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const quotationData = {
        ...
        clientName: selectedClient?.name || quotation.clientName,
        clientEmail: selectedClient?.email || quotation.clientEmail,
      ...
      await quotationService.updateQuotation(company.id, quotation.id, quotationData);
      onSuccess();
      onClose();
    } catch (err) {
      console.error('Failed to update quotation:', err);
      setError(language === 'pt' ? 'Erro ao atualizar orçamento' : 'Failed to update quotation');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="modal-container quotation-modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h2 className="modal-title">
            {language === 'pt' ? 'Editar Orçamento' : 'Edit Quotation'}
          </h2>
